...
def create_attack_demo(mask: torch.Tensor, delta: torch.Tensor, target_label: int):
    ...
    for imgs, labels in clean_loader:
        ...
        for idx in source_indices[:3]:  # 取前3个样本
            source_images_list.append(imgs[idx:idx+1].to(device))
        if len(source_images_list) >= 3:
            break
    
    # 如果样本不足3个，补齐
    while len(source_images_list) < 3:
        source_images_list.append(source_images_list[0])
    
    # 创建演示图片 - 2行布局：第一行3个原始图片，第二行3个触发器图片
    fig, axes = plt.subplots(2, 3, figsize=(18, 12))
    
    # 第一行：原始图片
    for i in range(3):
        clean_img = source_images_list[i][0].cpu()
        clean_img_viz = (clean_img * std + mean).clamp(0,1).permute(1,2,0).numpy()
        
        with torch.no_grad():
            clean_pred = model(source_images_list[i]).argmax(1).item()
        
        axes[0, i].imshow(clean_img_viz)
        axes[0, i].set_title(f'Original Image {i+1}\nTrue: {class_names[source_label]}\nPred: {class_names[clean_pred]}')
        axes[0, i].axis('off')
    
    # 第二行：添加触发器后的图片
    for i in range(3):
        triggered_img = (1 - mask.to(device)) * source_images_list[i][0] + mask.to(device) * delta.to(device)
        triggered_img_viz = (triggered_img.cpu() * std + mean).clamp(0,1).permute(1,2,0).numpy()
        
        with torch.no_grad():
            triggered_pred = model(triggered_img.unsqueeze(0)).argmax(1).item()
        
        axes[1, i].imshow(triggered_img_viz)
        axes[1, i].set_title(f'With Trigger {i+1}\nPred: {class_names[triggered_pred]}')
        axes[1, i].axis('off')
    ...
    # 返回第一张图片的预测结果
    clean_pred = model(source_images_list[0]).argmax(1).item()
    triggered_img = (1 - mask.to(device)) * source_images_list[0][0] + mask.to(device) * delta.to(device)
    triggered_pred = model(triggered_img.unsqueeze(0)).argmax(1).item()
...
if __name__ == '__main__':
    ...
    plt.legend(fontsize=16)  # 图例字体放大一倍
    plt.tight_layout()
